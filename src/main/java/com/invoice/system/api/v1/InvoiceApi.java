package com.invoice.system.api.v1;

import static com.invoice.system.common.Defaults.CRITERION;
import static com.invoice.system.common.Defaults.OFFSET;
import static com.invoice.system.common.Defaults.LIMIT;

import com.invoice.system.service.InvoiceObject;
import com.invoice.system.service.InvoiceCriteria;
...
public class InvoiceApi {
	...
	*/
	...
	public HttpEntity<List<InvoiceResource>> find(
		...
		List<InvoiceObject> invoices = invoiceService.find(criteria, offset, limit);
	...
	*/
	...
	public HttpEntity<InvoiceResource> add(@Valid @RequestBody InvoiceResource invoiceToAdd) {
		...
		InvoiceObject addedInvoice = invoiceService.add(toBean(invoiceToAdd));

		return new ResponseEntity<>(toResource(addedInvoice), HttpStatus.OK);
	}

	//---------------------------------------------------------------------------------------------
	private InvoiceObject toBean(InvoiceResource invoice) {
		return modelMapper.map(invoice, InvoiceObject.class);
	}	

	//---------------------------------------------------------------------------------------------
	private InvoiceResource toResource(InvoiceObject invoice) {
		return modelMapper.map(invoice, InvoiceResource.class);
	}	

	//---------------------------------------------------------------------------------------------
	private List<InvoiceResource> toResourceList(List<InvoiceObject> invoices) {
